...
if (!isset($_GET['id'])) {
    header("Location: Funcionario.php?fun=listar&status=" . urlencode("ID inválido!"));
    exit;
}

$id = (int)$_GET['id'];

// Passo 2: buscar dados do funcionario
...
if (!$cont) {
    header("Location: Funcionario.php?fun=listar&status=" . urlencode("Funcionario não encontrado!"));
    exit;
}
...
if (isset($_GET['conf']) && $_GET['conf'] === 'sim') {
    ...
    if ($stmtDel->execute()) {
        $status = "Funcionario excluído com sucesso!";
    } else {
        $status = "Erro ao excluir funcionario!";
    }
    ...
    header("Location: ../Funcionario.php?fun=listar&status=" . urlencode($status));
    exit;
}
...
<body>
    ...
    <script>
        ...
        function closeModal() {
            ...
            window.location.href = '../Funcionario.php?fun=listar';
        }

        // Confirmar exclusão
        function confirmDelete() {
            window.location.href = 'ConfirmarExclusaofun.php?id=<?= $cont->id ?>&conf=sim';
        }
    </script>
