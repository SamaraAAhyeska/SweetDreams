    ...
    public function cadastrar($cont)
    {
        try {
            $stmt = $this->con->prepare(
                "INSERT INTO cliente(nome, endereco, idade, cidade, telefone, cpf)
...
{
    try {
        $stmt = $this->con->prepare("DELETE FROM cliente WHERE id = :id");
    ...
    {
        try {
            $lista = $this->con->query("SELECT * FROM cliente WHERE id = " . $id);
    ...
    {
        try {
            $stmt = $this->con->prepare("SELECT * FROM cliente WHERE id = :id");
